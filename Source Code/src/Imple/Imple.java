...
return reg;
	...
	public boolean slog(sregbean sl) {
		...
		con=dbconn.create();
		 ...
		 try {
				PreparedStatement ps=con.prepareStatement("SELECT * FROM `bc04`.`staffreg` where team=? and email=? and password=? and status='Approved'");
	...
	public int tlf2(tlfilebean2 tf) {
		...
		{
			 con=dbconn.create();
			 PreparedStatement ps=con.prepareStatement("INSERT INTO bc04.stafffileadd VALUES(id,?,?,?,?,?,?,?,?,?,?,?)");
			 ps.setString(1, tf.getTitle());
			 ps.setString(2, tf.getDescription());
			 ps.setString(3, tf.getFilename());
			 ps.setString(4, tf.getFiletype());
